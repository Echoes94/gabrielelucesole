...
const TestimonialsSection = () => {
  ...
  return <section ref={sectionRef} className="section-padding bg-background relative overflow-hidden">
      ...
      {/* Floating elements */}
      <motion.div className="absolute top-1/4 left-10 w-20 h-20 border border-cyan/10 rounded-full" animate={{
      y: [0, -30, 0],
      rotate: 360
    }} transition={{
      duration: 15,
      ...
      <motion.div className="absolute bottom-1/4 right-10 w-16 h-16 border border-accent/10 rounded-full" animate={{
      y: [0, 30, 0],
      rotate: -360
    }} transition={{
      duration: 12,
      ...
      <div className="container-wide relative z-10">
        ...
        <AnimatedSection className="text-center mb-16">
          <motion.span className="inline-block px-4 py-2 text-xs font-sans uppercase tracking-[0.2em] text-cyan/70 border border-cyan/20 rounded-full mb-6" animate={{
          ...
          <motion.h2 className="font-display text-3xl md:text-5xl mb-4" initial={{
          opacity: 0,
          y: 30
        }} whileInView={{
          opacity: 1,
          ...
          <motion.p className="font-serif text-muted-foreground max-w-xl mx-auto" initial={{
          opacity: 0
        }} whileInView={{
          opacity: 1
        }} transition={{
        ...
        <div className="space-y-12 mb-12">
          {testimonials.map((testimonial, index) => <AnimatedSection key={index} delay={index * 0.2} direction={index % 2 === 0 ? "left" : "right"}>
              <motion.div className={`grid ${testimonial.videoId ? "lg:grid-cols-2" : "lg:grid-cols-1 max-w-3xl mx-auto"} gap-8 items-center gradient-border rounded-2xl p-6 md:p-8 bg-gradient-card`} whileHover={{
            scale: 1.02,
            ...
            duration: 0.4
                ...
                {testimonial.videoId && <motion.div className="relative aspect-video rounded-xl overflow-hidden bg-background/50" initial={{
              opacity: 0,
              scale: 0.9
            }} whileInView={{
              opacity: 1,
              ...
              once: true
                    ...
                    <iframe src={`https://drive.google.com/file/d/${testimonial.videoId}/preview`} width="100%" height="100%" allow="autoplay" className="absolute inset-0 w-full h-full" title={`Video testimonianza di ${testimonial.name}`} />
                  </motion.div>}

                {/* Content */}
                <div className={testimonial.videoId ? "" : "text-center"}>
                  {/* Quote icon with animation */}
                  <motion.div className={`w-10 h-10 rounded-full bg-cyan flex items-center justify-center mb-6 ${!testimonial.videoId ? "mx-auto" : ""}`} initial={{
                scale: 0,
                rotate: -180
              }} whileInView={{
                scale: 1,
                ...
                rotate: 10
                  ...
                  <motion.p className="font-serif text-muted-foreground leading-relaxed mb-6 italic" initial={{
                opacity: 0
              }} whileInView={{
                opacity: 1
              }} transition={{
                ...
                once: true
                  ...
                  {/* Author with stagger */}
                  <motion.div className={`flex items-center gap-3 ${!testimonial.videoId ? "justify-center" : ""}`} initial={{
                opacity: 0,
                x: -20
              }} whileInView={{
                opacity: 1,
                ...
                once: true
                    ...
                    <motion.div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan to-accent flex items-center justify-center" whileHover={{
                  scale: 1.1,
                  rotate: 360
                }} transition={{
                  duration: 0.5
                  ...
                  {/* CTA */}
                  <motion.div className={`mt-6 ${!testimonial.videoId ? "flex justify-center" : ""}`} initial={{
                opacity: 0,
                y: 20
              }} whileInView={{
                opacity: 1,
